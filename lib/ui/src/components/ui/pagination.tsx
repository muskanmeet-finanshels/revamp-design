...
/* Cast to plain React.FC so the React 18 DTS build is not blocked by lucide's React 19 peer types */
const ChevronDown   = _ChevronDown   as unknown as React.FC<any>;
const ChevronLeft   = _ChevronLeft   as unknown as React.FC<any>;
const ChevronRight  = _ChevronRight  as unknown as React.FC<any>;
const ChevronsLeft  = _ChevronsLeft  as unknown as React.FC<any>;
const ChevronsRight = _ChevronsRight as unknown as React.FC<any>;
...
export interface PaginationProps {
  ...
  /** Rows-per-page options. Defaults to [10, 12, 20, 50]. */
...
const DEFAULT_SIZE_OPTIONS = [10, 12, 20, 50];

/**
 * Responsive pagination bar.
 *
 * Mobile  — two rows: [Rows per page  |  Showing X to Y of Z]  /  [nav centred, no text labels]
 * Desktop — single row: [Rows per page]  …  [Showing X to Y of Z  +  full nav]
 */
